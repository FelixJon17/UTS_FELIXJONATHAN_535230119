...
async function getUsers(request, response, next) {
  try {
    // Parse request query parameters or set default values
    const pageNumber = parseInt(request.query.page_number) || 1;
    const pageSize = parseInt(request.query.page_size) || 0;
    const sortBy = request.query.sort || 'asc';
    const search = request.query.search || '';

    // Call the service to fetch users
    const users = await usersService.getUsers(pageNumber, pageSize, sortBy, search);
...
async function getUser(request, response, next) {
  try {
    // Call service to get user detail by id
